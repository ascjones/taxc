...
use chrono::{NaiveDate, NaiveDateTime, DateTime, Duration, Utc};
...
impl Prices {
    ...
    pub fn read_csv<'a, R>(reader: R) -> Result<Prices, Box<Error>> where R: Read {
    ...
    pub fn write_csv<W>(&self, writer: W) -> Result<(), Box<Error>> where W: Write {
        ...
        for (_pair, prices) in self.prices.iter() {
            for price in prices.iter() {
                let date_time =
                    DateTime::<Utc>::from_utc(price.date_time, Utc).to_rfc3339().clone();
                let record = Record {
...
impl Into<Price> for FiatPriceRecord {
    fn into(self) -> Price {
        let pair = CurrencyPair { base: GBP, quote: USD };
        // e.g. Jan 31, 2019
...
impl Into<Price> for EtherscanPriceRecord {
    fn into(self) -> Price {
        let pair = CurrencyPair { base: *ETH, quote: USD };
...
pub fn import_prices<R>(gbp_usd_file: R, btc_usd_file: R, eth_usd_file: R) -> Result<Prices, Box<Error>> where R: Read {
...
fn usd_to_gbp(pair: &CurrencyPair, gbp_usd_prices: &Vec<Price>, crypto_usd_prices: Vec<Price>) -> Vec<Price> {
    let mut gbp_prices = Vec::new();
    let mut last_price = None;
    for price in crypto_usd_prices.iter() {
        let price_date = price.date_time.date();
        let gbp_usd = gbp_usd_prices
            .iter()
            .find(|gbp_usd_price| {
//                println!("Searching {}", gbp_usd_price.date_time.date());
        ...
        let gbp_price = Price { pair: pair.clone(), date_time: price.date_time, rate: gbp_crypto_rate };
        gbp_prices.push(gbp_price);
    }
    gbp_prices
}

fn read_records<'de, T, R>(reader: R) -> Result<Vec<Price>, csv::Error> where R: Read, T: DeserializeOwned, T: Into<Price>, T: Clone {
    let mut rdr = csv::Reader::from_reader(reader);
...
fn parse_date(s: &str) -> NaiveDateTime {
    DateTime::parse_from_rfc3339(s)
        .expect(format!("Invalid date_time {}", s).as_ref()).naive_utc()
}