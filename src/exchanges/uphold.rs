...
fn crypto_account(currency: &Currency) -> Account {
    match currency.code().as_ref() {
        "BTC" => Account::new("Uphold", AccountKind::Crypto(Network::Bitcoin, None)),
        "ETH" => Account::new("Uphold", AccountKind::Crypto(Network::Ethereum, None)),
...
fn record_to_transactions(record: &Record) -> Vec<Transaction> {
    ...
    let entry = |acc: &Account, amt| Entry::new(acc.clone(), amt);

    let source_id = &record.id;
    let tx = move |dt, deb, cr, fee| Transaction::new(Some(source_id.clone()), dt, deb, cr, fee);
        ...
        "withdrawal" => {
            if currency::with_code(&credit_amt.currency.code()).is_some() {
                if debit_amt.currency == credit_amt.currency {
                    ...
                    let debit = entry(uphold_acct, debit_amt);
                    let credit = entry(uphold_acct, credit_amt);
                    let conversion = tx(date_time, debit, credit, fee);
                    // withdrawal to fiat
                    let debit = entry(uphold_acct, credit_amt);
                    let credit = entry(bank_account, credit_amt);
                    let withdrawal = tx(date_time, debit, credit, fee);
                    vec![conversion, withdrawal]
                }
            } else {
                panic!(
                    "Expecting withdrawal to known fiat currency. Debit {}, Credit {}, id {}",
                    debit_amt.currency.code(),
                    credit_amt.currency.code(),
                    source_id
                )
            }
        }
    ...
    R: Read,
{
    ...
    let mut txs: Vec<_> = result?.iter().flat_map(record_to_transactions).collect();
