...
use crate::{CurrencyPair, Prices, Price, display_amount, Entry, Year, BTC, ETH};
use crate::trades::{Trade, TradeKind, TradeKey};
...
impl TaxYear {
    ...
    fn proceeds(&self) -> Money {
        self.gains.iter().fold(Money::zero(GBP), |acc, g| acc + g.sell_value)
    }

    fn allowable_costs(&self) -> Money {
        self.gains.iter().fold(Money::zero(GBP), |acc, g|
            acc + g.allowable_costs)
    }
...
impl Gain {
    ...
    pub fn write_csv<W>(gains: &[Gain], writer: W) -> Result<(), Box<Error>> where W: Write {
    ...
    fn write_headers<W>(wtr: &mut csv::Writer<W>) -> Result<(), Box<Error>> where W : Write  {
        wtr.write_record(&[
            "Date",
            "Exchange",
            "Buy Asset",
    ...
    fn write_csv_record<W>(&self, wtr: &mut csv::Writer<W>) -> Result<(), Box<Error>> where W : Write {
        wtr.write_record(&[
...
pub fn calculate(trades: Vec<Trade>, prices: &Prices) -> Result<TaxReport, String> {
    ...
    let convert_to_gbp = |money: &Money, price: &Price, trade_rate: f64| {
        if money.currency == price.pair.base {
            ...
            money.convert_to(price.pair.base, trade_rate)
    ...
    let trades_with_prices = trades
        .iter()
        .map(|trade| {
            let price = get_price(trade, prices)
                .expect(format!("Should have price for buy: {} sell: {} at {}", trade.buy, trade.sell, trade.date_time).as_ref());
            (trade, price)
        })
        .collect::<Vec<_>>();
    ...
    let gains: Vec<Gain> = trades_with_prices
        ...
        .filter_map(|(trade, price)| {
            if trade.buy.currency != GBP {
                ...
                let buy_pool = pools.entry(trade.buy.currency)
...
//                special_rules_buy.sort_by(|(t1, _), (t2, _)| t1.date_time.cmp(&t2.date_time));
                ...
                let (main_pool_sell, special_allowable_costs) = special_rules_buy
                    .iter()
                    .fold((trade.sell, Money::zero(GBP)),
                            ...
                            if *remaining_buy_amount > Money::zero(remaining_buy_amount.currency) {
                                let (sell, special_buy_amt) =
                                    if *remaining_buy_amount <= main_pool_sell {
                                        (main_pool_sell - *remaining_buy_amount, *remaining_buy_amount)
                                    } else {
                ...
                let sell_pool = pools.entry(trade.sell.currency)
...
fn create_report(trades: Vec<Trade>, gains: Vec<Gain>, pools: HashMap<Currency, Pool>) -> TaxReport {
    ...
    TaxReport { trades: trades.to_vec(), years: tax_years, pools }
}
    ...
    let base =
        if quote == *BTC {
            ...
            *ETH
            ...
            panic!("Expected quote price to be BTC or ETH or GBP for trade at {}. quote {}, base {}",
...
fn uk_tax_year(date_time: NaiveDateTime) -> Year {
    ...
    let year = date.year();
    if date > ymd(year, 4, 5) && date <= ymd(year, 12, 31) { year + 1 } else { year }
}
...
mod tests {
    ...
    fn trade(dt: &str, kind: TradeKind, sell: Money, buy: Money, rate: f64) -> Trade {
        ...
        Trade { date_time, kind, sell, buy, rate, exchange: None }
    }
...
//        println!("{}", report);
        ...
        let ty_2018 = report.years.get(&2018).expect("Tax year 2018 should be calculated");
    ...
    fn hmrc_acquiring_within_30_days_of_selling_example() {
        let buy1 = trade("2018-01-01", TradeKind::Buy, gbp(200_000), btc(14_000), 14.285714286);
        ...
        let ty = report.years.get(&2019).expect("Tax year 2019 should be calculated");
...
//        println!("1: {}", disposal.trade);
    ...
    fn acquisition_within_30_days_greater_than_disposal_returned_to_pool() {
        ...
        let ty = report.years.get(&2019).expect("Tax year 2019 should be calculated");
        println!("GAINS {}", ty.gains.iter().map(|g| g.gain().to_string()).collect::<Vec<_>>().join(", "));
